...
if __name__ == '__main__':
    ...
    cold_pair = sc.textFile(test_file).map(lambda line: json.loads(line))                     .map(lambda item: (item['user_id'], item['business_id']))                     .filter(lambda item: item[0] not in user_dict).filter(lambda item: item[1] not in business_dict)                     .map(lambda pair: {"user_id": pair[0], "business_id": pair[1], "stars": grand_avg})
